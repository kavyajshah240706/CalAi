...
DEPTH_MODEL_ARCHITECTURE = os.environ.get('DEPTH_MODEL_ARCHITECTURE', './models/monovideo_fine_tune_food_videos.json')
DEPTH_MODEL_WEIGHTS = os.environ.get('DEPTH_MODEL_WEIGHTS', './models/monovideo_fine_tune_food_videos.h5')
SEGMENTATION_WEIGHTS = os.environ.get('SEGMENTATION_WEIGHTS', './models/mask_rcnn_food_segmentation.h5')
FOV = 70  # Camera field of view
GT_DEPTH_SCALE = 0.05  # Ground truth depth rescaling factor (calibrated down for realistic sizes)
MIN_DEPTH = 0.01
MAX_DEPTH = 10
...
def parse_results(output_dir):
    """
    Parse the results JSON and collect output images
    
    Args:
        output_dir: Directory containing output plots and results.json
    ...
    if os.path.exists(results_file):
        try:
            ...
            for segment in data.get('segments', []):
                ...
                results['total_volume'] += segment.get('volume', 0)
        except Exception as e:
    ...
    if os.path.exists(output_dir):
        for file in sorted(os.listdir(output_dir)):
            if file.endswith(('.png', '.jpg', '.jpeg')):
                results['output_images'].append(file)
...
def estimate_volume():
    ...
    # Generate output directory for this session
    output_dir = os.path.join(OUTPUT_FOLDER, session_id)
    os.makedirs(output_dir, exist_ok=True)
    
    actual_metadata_file = os.path.join(output_dir, 'results.json')
